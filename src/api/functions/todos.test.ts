...
import * as todoFunctions from "./todos";
...
describe("todo fetching", () => {
  test("returns all todos in a list", async () => {
    const todos = await todoFunctions.getAll({
      listId: LIST1_ID,
      userId: USER1_ID,
    });
  ...
  test("throws error when list does not exist", async () => {
    await expect(() =>
      todoFunctions.getAll({ listId: "nonexistent", userId: USER1_ID }),
...
describe("todo creation", () => {
  test("creates a todo", async () => {
    const result = await todoFunctions.create({
      userId: USER1_ID,
      data: {
        listId: LIST1_ID,
        text: "New Todo",
